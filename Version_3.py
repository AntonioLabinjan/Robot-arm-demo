...
MIN_BEND_DEG = 4.0
TERMINAL_RECT = pygame.Rect(PANEL_W + 18, 18, WIDTH - PANEL_W - 36, 210)
...
def cumulative_world_angles_deg(rel_angles_screen):
    total = 0
    values = []

    for angle in rel_angles_screen:
        total += -angle
        values.append(total)
...
def build_kinematic_entry(reason):
    ...
    symbolic_x = f"x_j = sum(i=1..j) L_i*cos(theta_1 + ... + theta_i), j <= {k}"
    symbolic_y = f"y_j = sum(i=1..j) L_i*sin(theta_1 + ... + theta_i), j <= {k}"

    lines = [
        f"[{timestamp}] {reason} | mode={mode} | k={k} | selected=J{selected_joint}",
        symbolic_x,
        symbolic_y,
...
def rebuild_terminal_lines():
    global equation_terminal_lines

    equation_terminal_lines = []

    for idx, entry in enumerate(equation_entries[-12:], start=max(1, len(equation_entries) - 11)):
        equation_terminal_lines.append(f"--- entry {idx} ---")
        equation_terminal_lines.extend(entry.splitlines())

    if not equation_terminal_lines:
        equation_terminal_lines = [
            "No equation snapshots yet.",
            "Click inside the workspace, press L, or click 'L: Log' to record the current kinematic equation.",
        ]


def append_kinematic_entry(reason):
    global equation_entries

    equation_entries.append(build_kinematic_entry(reason))

    if len(equation_entries) > 200:
        equation_entries = equation_entries[-200:]

    rebuild_terminal_lines()


def clear_kinematic_log():
    global equation_entries, export_status

    equation_entries = []
    export_status = "Log cleared."
    rebuild_terminal_lines()


def export_kinematics_log():
    global export_status

    if not equation_entries:
        append_kinematic_entry("AUTO SNAPSHOT BEFORE EXPORT")

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = Path(f"kinematics_export_{timestamp}.txt")

    header = [
        "Robot Kinematics Lab - Kinematic Equation Export",
        f"Exported at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"Entries: {len(equation_entries)}",
        "",
        "Coordinate convention: x positive right, y positive upward from robot base.",
    ...
    export_status = f"Exported: {filename.name}"


def get_wrapped_terminal_lines(max_width):
    wrapped = []

    for line in equation_terminal_lines:
        if not line:
            wrapped.append("")
            continue

        pieces = wrap_text(line, small_font, max_width)
        wrapped.extend(pieces if pieces else [line])

    return wrapped


def draw_kinematic_terminal(surface):
    rect = TERMINAL_RECT
    surf = pygame.Surface((rect.w, rect.h), pygame.SRCALPHA)
    pygame.draw.rect(surf, (4, 8, 15, 222), surf.get_rect(), border_radius=12)
    pygame.draw.rect(surf, (90, 210, 255, 145), surf.get_rect(), 1, border_radius=12)
    surface.blit(surf, rect.topleft)

    draw_text(surface, "Kinematic equation terminal", rect.x + 14, rect.y + 10, TEXT, small_font)
    draw_text(surface, "click workspace -> snapshot | L log | E export | X clear", rect.x + 14, rect.y + 30, MUTED, small_font)

    btn_log_eq.draw(surface, False)
    btn_export_txt.draw(surface, False)
    btn_clear_log.draw(surface, False)
    ...
    visible = get_wrapped_terminal_lines(content_w)[-max_lines:]

    for i, line in enumerate(visible):
        color = GOOD if line.startswith("---") else TEXT
        draw_text(surface, line, content_x, content_y + i * line_h, color, small_font)

    draw_text(surface, export_status, rect.x + 14, rect.bottom - 21, GOOD if export_status.startswith("Exported") else MUTED, small_font)
...
equation_entries = []
equation_terminal_lines = []
export_status = "TXT export ready."
pending_equation_log = None
rebuild_terminal_lines()
...
btn_log_eq = Button(TERMINAL_RECT.right - 313, TERMINAL_RECT.y + 9, 88, 28, "L: Log")
btn_export_txt = Button(TERMINAL_RECT.right - 218, TERMINAL_RECT.y + 9, 96, 28, "E: Export")
btn_clear_log = Button(TERMINAL_RECT.right - 115, TERMINAL_RECT.y + 9, 88, 28, "X: Clear")
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if btn_log_eq.handle_event(event):
            append_kinematic_entry("MANUAL BUTTON LOG")
            ui_clicked = True

        if btn_export_txt.handle_event(event):
            export_kinematics_log()
            ui_clicked = True

        if btn_clear_log.handle_event(event):
            clear_kinematic_log()
            ui_clicked = True
        ...
        if event.type == pygame.KEYDOWN:
            if event.key in (pygame.K_ESCAPE, pygame.K_q):
                ...
                append_kinematic_entry("MANUAL KEY LOG")
        ...
        if (
            ...
            and event.button == 1
            and event.pos[0] > PANEL_W
            and not ui_clicked
            and not TERMINAL_RECT.collidepoint(event.pos)
        ):
            if mode in (MODE_END_IK, MODE_JOINT_IK):
                set_active_target(screen_to_world(event.pos))
                pending_equation_log = "WORKSPACE CLICK TARGET"
                drag_workspace = True
            else:
                set_selected_joint(select_nearest_joint(model_points, event.pos))
                pending_equation_log = "WORKSPACE CLICK SELECT JOINT"
    ...
    if pending_equation_log:
        append_kinematic_entry(pending_equation_log)
        pending_equation_log = None
    ...
    draw_kinematic_terminal(screen)
